...
export const DialogVar: FC<Props> = ({ prompt, onSubmit, onClose }) => {
  const vv = prompt.vars
    ? prompt.vars.map((variable) => ({ key: variable, value: "" }))
    : [];
    ...
    >
      <div
        ...
        className="dark:border-netural-400
                inline-block max-h-[400px] max-w-[600px] transform overflow-y-auto
                rounded-lg border border-gray-300 bg-white px-4 pb-4 pt-5 text-left align-bottom
                shadow-xl
      ...
      >
        ...
        {updatedVariables.map((variable, index) => (
          <div className="mb-2 flex " key={index}>
            <div className="text-gray align-right mb-1 w-[150px] text-sm font-bold">
              {variable.key}
            </div>
