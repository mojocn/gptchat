...
export function DialogSession({ session }: { session: Session }) {
  const { upsertSession }: ChatState = useChatStore();
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        ...
        <Button title="Settings" variant="ghost">
          <IconAdjustments />
        </Button>
      </DialogTrigger>
      <DialogContent className="max-w-[95%] xl:w-[1000px]">
        <DialogHeader>
          ...
          <DialogDescription>Set Session configuration</DialogDescription>
        </DialogHeader>

        <Form {...formData}>
          <form
            onSubmit={formData.handleSubmit(onSubmit, onErrors)}
