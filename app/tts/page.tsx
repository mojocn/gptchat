  ...
  persist(
    (set, get) =>
      ({
        result: undefined,
        resultAll: [],
        setResult: (v: TtsResult) =>
          set({ resultAll: [...get().resultAll, v], result: v }),
...
export default function Tts() {
  ...
  async function init() {
    ...
    rec.recognized = (
      ...
      const pronunciationAssessmentResult = toTtsResult(
        PronunciationAssessmentResult.fromResult(r),
      );
  ...
  return (
    <div className="mx-auto max-w-[36rem] overflow-hidden">
      <h3 className="my-2 text-center">{`${idx + 1}/${lines.length}`}</h3>
      <p className="my-4 text-center font-mono text-gray-400 dark:text-gray-200 ">
        {speechTxt()}
      </p>
      <p className="my-4 text-center font-mono text-gray-400 dark:text-gray-200">
        {result?.Lexical}
      ...
      {result && result.PronunciationAssessment && (
        <PronounceScore score={result?.PronunciationAssessment} />
      )}
      ...
      <div className="align-center absolute bottom-0 left-0 right-0  m-4 flex items-center justify-center gap-4">
        <CaButton onClick={doIndxDesc} loading={loading} theme="primary">
          {" "}
          <IconChevronLeft />
        </CaButton>

        {recognizing ? (
          <CaButton onClick={recognizerStop} theme="danger">
            {" "}
        ...
        {/*          type="warning"*/}
        {/*> <IconEar/></CaButton>*/}
        <CaButton onClick={doSpeak} loading={loading} theme="primary">
          {" "}
          <IconVolume />
        </CaButton>

        <CaButton
          onClick={doSendResultToCloud}
          loading={loading}
        ...
        <CaButton onClick={clearResult} loading={loading} theme="primary">
          {" "}
          <IconTrash />
        </CaButton>
        <CaButton onClick={doIndxInc} loading={loading} theme="primary">
          {" "}
          <IconChevronRight />
        </CaButton>
      </div>
