...
export function WordTag(props: Word) {
    ...
    return (<div>
        <p className={`font-semibold ${props.PronunciationAssessment.ErrorType === 'None' ? 'text-indigo-800' : 'text-red-800'}`}
           title={`${props.PronunciationAssessment.AccuracyScore}`}>{props.Word}</p>
        ...
        <div className="flex justify-around">
            {props.Phonemes?.map((s, i) => {
                return <span key={i} className={scoreColor(s.PronunciationAssessment.AccuracyScore)}
                             title={`${s.PronunciationAssessment.AccuracyScore}`}>{s.Phoneme}</span>
            })}
        </div>
